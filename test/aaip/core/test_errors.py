...
class TestErrorTypes:
    ...
    def test_delegation_error(self):
        ...
        error = DelegationError(AAIPErrorCode.INVALID_DELEGATION, "Invalid delegation format")
    ...
    def test_constraint_violation_error(self):
        ...
        error = ConstraintError(AAIPErrorCode.CONSTRAINT_VIOLATED, "Amount exceeds limit")
        assert "Amount exceeds limit" in str(error)
        assert isinstance(error, AAIPError)
    
    def test_identity_verification_error(self):
        """Test identity verification errors."""
        error = ValidationError(AAIPErrorCode.IDENTITY_VERIFICATION_FAILED, "Identity verification failed")
...
class TestErrorCreation:
    ...
    def test_missing_field_error(self):
        ...
        error = DelegationError(AAIPErrorCode.MISSING_REQUIRED_FIELD, "Missing field: signature")
    ...
    def test_delegation_expired_error(self):
        ...
        error = AuthorizationError(AAIPErrorCode.DELEGATION_EXPIRED, "Delegation expired at: 2025-01-01T00:00:00Z")
    ...
    def test_delegation_not_valid_yet_error(self):
        ...
        from aaip.core import AuthorizationError
        error = AuthorizationError(AAIPErrorCode.DELEGATION_NOT_YET_VALID, "Delegation not valid until: 2025-12-31T23:59:59Z")
    ...
    def test_scope_insufficient_error(self):
        ...
        from aaip.core import AuthorizationError
        error = AuthorizationError(AAIPErrorCode.SCOPE_INSUFFICIENT, "Required scope payments:authorize not in granted scopes: email:send")
...
class TestErrorHandlingScenarios:
    ...
    def test_identity_verification_errors(self):
        ...
        assert validate_identity_format("did:example:123", "did") == True
        assert validate_identity_format("user@example.com", "oauth") == True
        assert validate_identity_format("", "custom") == False  # Empty identity invalid
        assert validate_identity_format("invalid", "did") == False  # Invalid DID format
...
class TestErrorRecovery:
    ...
    def test_error_chaining(self):
        ...
        try:
            ...
            chained_error = DelegationError(AAIPErrorCode.INVALID_DELEGATION, f"Failed to process delegation: {e}")
