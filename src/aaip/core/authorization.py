...
from .exceptions import ConstraintError, AAIPErrorCode
...
def _validate_max_amount(constraint: Dict[str, Any], request_data: Dict[str, Any]) -> None:
...
def _validate_time_window(constraint: Dict[str, Any], request_data: Dict[str, Any]) -> None:
    """Validate time window constraints."""
    now = datetime.now(timezone.utc)

    if 'start' in constraint:
        try:
            start_time = datetime.fromisoformat(constraint['start'].replace('Z', '+00:00'))
            if now < start_time:
                raise ConstraintError(
                    AAIPErrorCode.CONSTRAINT_VIOLATED,
            ...
            raise ConstraintError(
                AAIPErrorCode.INVALID_FIELD_FORMAT,
                f"Invalid start time format: {e}"
            )
    ...
    if 'end' in constraint:
        try:
            ...
            raise ConstraintError(
                AAIPErrorCode.INVALID_FIELD_FORMAT,
                f"Invalid end time format: {e}"
            )


def _validate_allowed_domains(constraint: list[str], request_data: Dict[str, Any]) -> None:
...
def _validate_blocked_domains(constraint: list[str], request_data: Dict[str, Any]) -> None:
...
def _validate_blocked_keywords(constraint: list[str], request_data: Dict[str, Any]) -> None:
...
def validate_constraints(constraints: Dict[str, Any], request_data: Dict[str, Any]) -> bool:
    """
...
def check_delegation_authorization(
    ...
    now = datetime.now(timezone.utc)
    try:
        expires_at = datetime.fromisoformat(delegation.delegation.expires_at.replace('Z', '+00:00'))
        if now >= expires_at:
            return False
    except ValueError:
        return False
...
def verify_delegation(
    delegation: Union[Dict[str, Any], Delegation],
    ...
    try:
        ...
        required_payload_fields = ['id', 'issuer', 'subject', 'scope', 'issued_at', 'expires_at']
        ...
        # Parse timestamps
        try:
            issued_at = datetime.fromisoformat(delegation_payload['issued_at'].rstrip('Z')).replace(tzinfo=timezone.utc)
            expires_at = datetime.fromisoformat(delegation_payload['expires_at'].rstrip('Z')).replace(tzinfo=timezone.utc)

            if 'not_before' in delegation_payload:
                not_before = datetime.fromisoformat(delegation_payload['not_before'].rstrip('Z')).replace(tzinfo=timezone.utc)
                if now < not_before:
                    return False
...
def create_signed_delegation(
    ...
    return AAIPCrypto.create_signed_delegation(delegation_payload.to_dict(), issuer_private_key)
